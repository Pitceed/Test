...
console.log({mobileMenu,menuButton, body})
...
function createSlider(selector: string) {
    /**
     *
     */
    const rootEl = document.querySelector<HTMLElement>(selector)

    if (!rootEl) {
        throw new Error(`Slider root not found by ${selector}`)
    }

    const slides = rootEl.querySelectorAll('.group__card')
    const pagination = rootEl.querySelectorAll('.pagination .dot')
    const pageIndicator = rootEl.querySelector<HTMLElement>('.pagination .panel')
    const slidesContainer = slides[0].parentElement

    console.log(`Slider initialized`, {
        selector, rootEl, slides, pagination, pageIndicator
    })
...
const sliderSelectors: Array<string> = [
    '.recruitment__block', '.candidate__block', '.conclusion__block'// ....
]

sliderSelectors.forEach(createSlider)


// const slides = document.querySelectorAll('.group__card')
